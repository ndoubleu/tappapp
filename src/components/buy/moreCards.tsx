    ...
    return (
      <div className='grid grid-cols-4 grid-rows-2 w-full p-4 bg-block rounded-[24px] gap-2'>
        ...
        <div className="col-span-2 flex flex-col w-full">
          <span className="text-base font-bold">AMD Saphire</span>
          <span className="text-sm">Power: 0.09 GH/s</span>
        </div>
        <div className="flex flex-nowrap justify-center gap-2 items-center cursor-pointer">
          <div className="rounded-full bg-blockLighter p-2" onClick={()=>{if(count>0){setCount(count-1)}}}>
            -
          </div>
          <span>{count}</span>
          <div className="rounded-full bg-white text-block p-2 cursor-pointer" onClick={()=>{setCount(count+1)}}>
            +
          </div>
        </div>
